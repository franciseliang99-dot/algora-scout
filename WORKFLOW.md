...
- ~~**coollabsio/coolify** — R1 PHP/Blade~~ **解锁 by V0.1.8** (2026-04-29 user-explicit PHP Tier 1 lift): PHP 6.8M + Blade 1.6M; Algora 38 open / $3,693 + $1,382 历史。现已是 PHP 主要 scout target (见 PHP block 上面)。**V0.1.12 caveat (2026-04-30 早)**: Algora-标 issue 池实测 swarm 锁 — coolify#7458 9 closed PR + GitHub Actions 自动 close + algora-pbc bot 写 "AI PRs will not receive review" 验证 (shipped-log V0.1.12 abort row + takeaway #17), 整 Algora-标 issue 池 cold-account 永久零机会; **但 R8 repo 健康 (founder + ShadowArcanist + 4/20 真外部 merger 2026-04-22~30), V0.1.4 模式 unassigned bug 池仍可探,PHP 0-exp 40-80h ramp-up 投入风险待用户决策**。Revert path: `git revert <V0.1.8 sha>` 恢复 R1 PHP poison; **V0.1.12 swarm caveat 独立于 V0.1.8 unlock,记录的是 swarm 现实, 不被 V0.1.8 revert 影响**。
...
- **PR-level bounty (`/pull/N` URL 类)** — algora.io/bounties 列 `github.com/<o>/<r>/pull/N` 而非 `/issues/N` URL = bounty 挂在已 OPEN 的 PR 上, reward 给 PR author 不是 cold-account 可竞争入口 (V0.1.12 verified turso #6583/#6626 + activepieces #12262 + coder/registry #772 全 MERGED+Rewarded 仍列 — 与 V0.1.7 takeaway #10 24-72h sync delay 同质)。**Pre-flight grep 必区分**: `grep -oE 'github\.com/[^"]+/issues/[0-9]+'` (issue-level, 可竞争) vs `grep -oE 'github\.com/[^"]+/pull/[0-9]+'` (PR-level, 直接跳)。例外: PR-into-PR 模式 — 当 PR author 在 thread 公开邀请 co-author commits 时入口仍开 (Step 6 variant + grundmanise/mastra#1 V0.1.3 precedent)。
- **"auto-close + bounty hunter swarm" 模式 (新 poison 类型, V0.1.12 takeaway #17)** — 低 stakes Algora-标 issue 引来 7+ AI/低质量 PR + GitHub Actions / 内部规则自动 close + maintainer 0 回应 → bounty 永远不发。**识别信号 (任三同满 = 整池跳)**: ① 单 issue 5+ closed fix PR; ② 评论流多 "/attempt" + AI/低质量 reply; ③ maintainer 0 回应 close PR; ④ Algora-标的 issue 有 algora-pbc bot 写 "AI PRs will not receive review"。已 verified: coollabsio/coolify Algora-标 issue 池 (#7458 9 closed PR)。**与 keephq 高奖金抢占失控池同质 vector 不同**: keephq = 高奖金引人, swarm = 低奖金 + AI swarm + auto-close。
